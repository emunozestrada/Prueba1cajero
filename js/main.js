...
const keycancel = document.querySelector("#btn-cancel");
const keycorrection = document.querySelector("#btn-delete");
...
const validatePin = (pin_keys) => {
  if (pin_keys.toLocaleString().replaceAll(",", "") === "0000") {
    ...
    keycorrection.addEventListener("click", function(){
      screen.innerText = "";
      setTimeout(() => {
        screen.innerText = "Ingrese su pin nuevamente";
        state = "login";
      }, 200);
    });
...
keycancel.addEventListener("click", function(){
  screen.innerText = "Operación cancelada!";
  setTimeout(() => {
    screen.innerText = "Retire su tarjeta";
    state = "login";
  }, 2500);
  setTimeout(() => {
    screen.innerText = "Bienvenido al ATM";
